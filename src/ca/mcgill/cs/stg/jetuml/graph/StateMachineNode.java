...
/**
 * An state machine node in a state machine diagram that can be composed
 * of two compartments: top (for the name), and bottom (for actions).
 */
public class StateMachineNode extends RectangularNode implements ChildNode
{
	protected static final int DEFAULT_COMPARTMENT_HEIGHT = 10;
	protected static final int DEFAULT_WIDTH = 110;
	protected static final int DEFAULT_HEIGHT = 70;
	
	protected MultiLineString aName;
	protected MultiLineString aActions;   
	
	private ParentNode aContainer;
	

	/**
     * Construct an state machine node with a default size and
     * the text <<state of machine>>.
	 */
	public StateMachineNode()
	{
		aName = new MultiLineString(true);
		aName.setText("\u00ABstate of machine\u00BB\n");
		aName.setJustification(MultiLineString.CENTER);
		aActions = new MultiLineString();
		aActions.setJustification(MultiLineString.LEFT);
	...
	protected Rectangle2D computeBottom(Graphics2D pGraphics2D)
	{
		...
		Rectangle2D bottom = aActions.getBounds(pGraphics2D);
	...
	{
		return !aActions.getText().isEmpty();
	}
     ...
     * @param pName the state machine name
	 */
     ...
     * @param pActions the actions of this state machine
	 */
	public void setActions(MultiLineString pActions)
	{
		aActions = pActions;
	}
     ...
     * @return the actions of this state machine
	 */
	public MultiLineString getActions()
	{
		return aActions;
	}
	...
	{
		StateMachineNode cloned = (StateMachineNode)super.clone();
		cloned.aName = aName.clone();
		cloned.aActions = aActions.clone();
		return cloned;
	}
